...
from commonroad.scenario.scenariocomplement import ScenarioWrapper
from commonroad.visualization.mp_renderer import MPRenderer
from matplotlib.animation import FuncAnimation
from ego_vehicle import EgoVehicle

def create_video(scenario: ScenarioWrapper,
                 output_folder: str,
    ...
    steps = []
    for step in scenario.obstacles.keys():
        for veh in scenario.obstacles[step].values():
            if isinstance(veh, DynamicObstacle):
                steps.append(step)
                break

    frame_count = max(steps) - min(steps) + 1 + frame_count_padding
    ...
    def init_plot():
        ...
        if dynamic_obstacles_ego is not None:
            rnd.draw_list(dynamic_obstacles_ego, draw_params={'time_begin': 0, 'time_end': 0, "axis_visible": False,
                                          'dynamic_obstacle': {"show_label": False, 'vehicle_shape': {"occupancy": {"shape": {"rectangle": {
                                              "facecolor": "red"}}}}}})
        scene = scenario.commonroad_scenario_at_time_step_all(min(steps))
        scene.draw(renderer=rnd, draw_params={'time_begin': 0, 'time_end': 0})
        if colors is not None:
            for c, ob_ids in colors.items():
                obs = [o for o in scene.dynamic_obstacles if o.obstacle_id in ob_ids]
                rnd.draw_list(obs,
                                 draw_params={'time_begin': 0, 'time_end': 0, 'dynamic_obstacle':{"show_label": False, 'shape':{'facecolor': c},
    ...
    def animate_plot(frame):
        ...
        if planning_problem_set is not None:
            planning_problem_set.draw(rnd)
        scene = scenario.commonroad_scenario_at_time_step_all(frame+min(steps))
        rnd.draw_list(scene.dynamic_obstacles,
                 draw_params={'time_begin': 0, 'time_end': 0, 'dynamic_obstacle':{"show_label": True}})
        rnd.draw_list(scene.static_obstacles,
                 draw_params={'time_begin': 0, 'time_end': 0, 'dynamic_obstacle':{"show_label": False}})
        if colors is not None:
            for c, ob_ids in colors.items():
                obs = [o for o in scene.dynamic_obstacles if o.obstacle_id in ob_ids]
                rnd.draw_list(obs,
                              draw_params={'time_begin': 0, 'time_end': 0,
                                'dynamic_obstacle':{"show_label": False, 'shape': {'facecolor': c},
                                                    'vehicle_shape': {"occupancy": {"shape": {"rectangle": {
                                                        "facecolor": c}}}}
                                 }})
